#!/usr/bin/env python3
"""Conservatively deduplicate normalized Standard Ebooks JSONL records.

This pass removes only exact full-text duplicates from the derived output.
Same-work clusters, alternate editions, translations, and volumes are retained
and reported for review. It also flags cross-source overlaps with Gutenberg
and Internet Archive records when the same work_id appears in both.
"""
...
DEFAULT_INPUT = Path("corpus/normalized/jsonl/standard_ebooks.jsonl")
DEFAULT_OUTPUT = Path("corpus/normalized/jsonl/standard_ebooks.deduped.jsonl")
DEFAULT_REPORT = Path("corpus/reports/standard_ebooks_dedup.md")
DEDUPER_ID = "scripts/dedup/dedup_standard_ebooks.py"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Deduplicate normalized Standard Ebooks JSONL conservatively, "
...
def render_report(
    ...
    lines = [
        ...
        (
            ...
            "`corpus/normalized/jsonl/standard_ebooks.jsonl`. The original normalized file was "
            "left unchanged."
        ),
        "",
        "## Summary",
    ...
    lines.extend(
        [
            ...
            "## Cross-Source Notes",
            "",
            "Several Standard Ebooks versions share work_ids with existing Project Gutenberg "
            "records. These are not flagged as exact duplicates here because they were normalized "
            "from different source files and the text may differ (different typo corrections, "
            "metadata normalization, etc.). Training data preparation should apply version "
            "selection or down-weighting for same-work cross-source records.",
            "",
            "The `plato_dialogues_jowett_se` record is a large collection that contains the "
            "full text of many dialogs also present individually in Gutenberg records. This "
            "collection record should be treated as a superset and weighted carefully relative "
            "to individual dialog records.",
            "",
            "## Notes",
            "",
