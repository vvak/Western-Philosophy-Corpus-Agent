#!/usr/bin/env python3
"""Acquire raw Project Gutenberg artifacts listed in the version manifest.

This script is intentionally source-specific and acquisition-only. It does not
normalize, segment, deduplicate, or remove Project Gutenberg boilerplate.
"""
...
DEFAULT_OUTPUT_DIR = Path("corpus/raw/gutenberg")
DEFAULT_FORMATS = ("txt", "epub")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Download raw Project Gutenberg text/EPUB artifacts for "
            "high-confidence records in corpus/manifests/versions.jsonl."
        )
    )
    parser.add_argument(
        "--manifest",
    ...
    parser.add_argument(
        ...
        help=f"Raw Project Gutenberg output directory. Default: {DEFAULT_OUTPUT_DIR}",
    )
    parser.add_argument(
        "--formats",
        default=",".join(DEFAULT_FORMATS),
        help="Comma-separated artifact formats to acquire: txt,epub. Default: txt,epub",
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=60.0,
        help="Network timeout in seconds for each download request. Default: 60.",
    )
    parser.add_argument(
        "--sleep",
        type=float,
        default=0.5,
        help="Delay in seconds between network requests in download mode. Default: 0.5.",
    )
...
def parse_formats(value: str) -> tuple[str, ...]:
    ...
    if unknown:
        raise SystemExit(f"Unsupported --formats value(s): {', '.join(unknown)}")
    if not formats:
        raise SystemExit("--formats must include at least one of: txt, epub")
    return formats
...
def candidates_for_record(record: dict[str, Any], formats: tuple[str, ...]) -> list[DownloadCandidate]:
    ...
    if "txt" in formats:
        candidates.append(
            DownloadCandidate(
                format_name="txt",
                url=f"https://www.gutenberg.org/cache/epub/{source_id}/pg{source_id}.txt",
    ...
    if "epub" in formats:
        candidates.append(
            DownloadCandidate(
                format_name="epub",
                url=f"https://www.gutenberg.org/ebooks/{source_id}.epub.noimages",
                filename=f"{version_id}.epub",
            )
        )
...
def raw_dir_for_record(output_dir: Path, record: dict[str, Any]) -> Path:
    source_id = str(record["source_id"])
    version_id = str(record["version_id"])
    return output_dir / f"pg_{source_id}_{version_id}"


def dry_run(records: list[dict[str, Any]], formats: tuple[str, ...], output_dir: Path) -> int:
    artifact_count = 0
    ...
    for record in records:
        ...
        for candidate in candidates_for_record(record, formats):
            artifact_count += 1
            print(f"  [{candidate.format_name}] {candidate.url} -> {candidate.filename}")

    print(f"\nPlanned artifacts: {artifact_count}")
    return 0


def download(records: list[dict[str, Any]], formats: tuple[str, ...], args: argparse.Namespace) -> int:
    ...
    print(
        f"Completed Project Gutenberg acquisition: {downloaded} downloaded, "
...
def main() -> int:
    ...
    if args.dry_run:
        return dry_run(selected, formats, args.output_dir)
    return download(selected, formats, args)
