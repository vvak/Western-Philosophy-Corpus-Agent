#!/usr/bin/env python3
"""Acquire raw Standard Ebooks EPUB artifacts listed in the version manifest.

Standard Ebooks provides clean, CC0-licensed EPUBs from public-domain texts.
The EPUB download URL pattern is:

  https://standardebooks.org/ebooks/{source_id}/downloads/{slug}.epub

where {slug} is the source_id with '/' replaced by '_'.
...
DEFAULT_OUTPUT_DIR = Path("corpus/raw/standard_ebooks")
SE_EBOOKS_BASE = "https://standardebooks.org/ebooks"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Download raw Standard Ebooks EPUB artifacts for "
            "high-confidence standard_ebooks records in corpus/manifests/versions.jsonl."
        )
    )
    parser.add_argument(
        "--manifest",
    ...
    parser.add_argument(
        ...
        help=f"Raw Standard Ebooks output directory. Default: {DEFAULT_OUTPUT_DIR}",
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="Delay in seconds between network requests. Default: 2.0.",
    )
...
def epub_slug(source_id: str) -> str:
    """Convert source_id to the EPUB filename slug (replace '/' with '_')."""
    return source_id.replace("/", "_")


def epub_url(source_id: str) -> str:
    """Construct the Standard Ebooks EPUB download URL for a given source_id.

    The ?source=download query parameter is required: without it, the server
    returns an HTML "Your Download Has Started!" page with a meta-refresh
    redirect instead of the binary EPUB file.
    """
    slug = epub_slug(source_id)
    return f"{SE_EBOOKS_BASE}/{source_id}/downloads/{slug}.epub?source=download"
...
def raw_dir_for_record(output_dir: Path, record: dict[str, Any]) -> Path:
    ...
    version_id = str(record["version_id"])
    slug = epub_slug(source_id)
    return output_dir / f"se_{slug}_{version_id}"
...
def download(records: list[dict[str, Any]], args: argparse.Namespace) -> int:
    ...
    print(
        f"Completed Standard Ebooks acquisition: {downloaded} downloaded, "
